        ...
        function obj = readInData(par_ui)
            ...
            if exist([ext(2:end) '_wc_reader'],'file')
                ...
                if ~with_par                                                                                            %if didn't load sr from previous results 
                    if isempty(sr)
                        disp('Wave_clus didn''t find a sampling rate in raw data so temporarily use the SR in set_parameters.m until paramters are updated with your input.')  %use default sr (from set_parameters) 
                    else
