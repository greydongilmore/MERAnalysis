...
% open parallel pool, if parallel input is true
    ...
    if run_par_for == true
        parfor fnum = 1:length(filenames)
            ...
            fprintf('%d of %d ''times'' files finished.',count_new_times(initial_date, filenames),length(filenames) )
            fprintf('\n')
        end
    else
...
function do_clustering_single(filename,min_spikes4SPC, par_file, par_input,fnum)
    ...
    par = struct;
    par = update_parameters(par,par_file,'relevant');
...
for i = 1:length(filenames)
    fname = filenames{i};
    temp = strsplit(fname,'\');
    FileInfo = dir(['times_' temp{1}(1:end-11) '.mat']);
